...
public class Room implements AutoCloseable {
    private static Server server;// used to refer to accessible server functions
    private String name;
	...
	if (exists) {
		...
		if (Server.run.equals("n") && getName().equals("Lobby")) {
	...
	if (name == null || name.equalsIgnoreCase(Server.LOBBY)) {
	    return;
	}
	try {
	...
	//nrm6, 12/13/23 flip and roll methods
	...
	try {
	    ...
	    case MUTE: 
	    	...
	    	sendPrivateMessage(client, ": muted ", muteList);
